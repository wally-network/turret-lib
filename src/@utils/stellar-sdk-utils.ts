 ...
 * @returns {boolean}.
 */
export function verifyTxSignedBy(transaction: any, accountID: any) {
 ...
 */
export function gatherTxSigners(transaction: any, signers: any) {
...
export async function processFeePayment(env: EnvProps, xdr: any, min: any, max: any) {
  ...
  if (min) {
    if (new BigNumber(op.amount).lessThan(min)) {
      throw { message: `Fee payment too low. Min = ${min}` }
    }
  }

  if (max) {
    if (new BigNumber(op.amount).greaterThan(max)) {
 ...
 */
async function processTransaction(horizonUrl: any, transaction: any) {
