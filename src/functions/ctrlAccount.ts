...
import { Turret, TxFunctions } from './';
import { parse } from '@iarna/toml'


export class ctrlAccount {
  static async heal({ request, params, env }: { request: Request, params: { ctrlAccount: string }, env: EnvProps}) {
      ...
      async (signer: any) => ({
        ...
        .then((account) => {
          ...
          return (
            ...
            ? TxFunctions.get({params: txFunctionHash, env: { TX_FUNCTIONS }}) 
    ...
    .filter((signer: any) => signer.toml && signer.weight)
    .sumBy('weight')
    .value()
    ...
    const { txFunctionSignerPublicKey, txFunctionSignerSecret } = metadata as { txFunctionSignerPublicKey: string, txFunctionSignerSecret: string } 
